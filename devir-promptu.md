...
- **Sepet "kasa altı" önerileri:** `CartDrawer` açılınca `/api/suggestions?exclude=<productId,...>` → `getCartSuggestions()` (aktif + stokta, sepettekiler hariç, kampanyalı/indirimli önce, en ucuz stok varyantı hızlı-ekle hedefi). Toplam bloğunun hemen üstünde yatay kaydırmalı şerit, "+ Ekle" butonu (`add_to_cart` olayı da gönderir). Sepet öğesine `deal`/`compareAtCents` dahil ekler.
