...
- **Ürün kampanyası "X alana Y" / BOGO (migration 0019):** `products.deal_buy_qty` / `deal_get_qty` / `deal_get_percent` (üçü birlikte null = kampanya yok). SATIR BAZLI: aynı varyanttan her (buy+get) adette get_qty adet, satır fiyatının %percent'i kadar iner. `create_order` içinde hesaplanır (yine DROP + yeniden yaratıldı; dönüşe `deal_discount_cents` eklendi). Kuponla birlikte uygulanır — kampanya + kupon toplamı ara toplamı geçemez (kupon kırpılır). `orders.deal_discount_cents` sütunu. Önizleme: `src/lib/pricing.ts` `lineDealDiscountCents()` (SQL ile aynı formül) — sepet öğesine `deal` alanı eklendi (`cart-store.ts`), `ProductCard`/`ProductDetailClient` eklerken doldurur + rozet gösterir (`dealBadgeText`). `CheckoutForm` özetinde "Kampanya indirimi" satırı. Admin: `ProductForm` "Kampanya" kartı (şablon butonları: 1 Alana 1 Bedava / 2 Alana 1 Bedava / 2. Ürün %50). Admin sipariş detayı + e-posta indirim satırını gösterir. `/api/checkout`: indirimler sonrası tutar 0 ise kart ödemesi engellenir (havale önerilir).
- **Favicon:** `src/app/icon.svg` + `apple-icon.svg` (yeşil yaprak). `layout.tsx` `generateMetadata` yüklü site logosu varsa onu favicon yapar (kare olmalı), yoksa SVG fallback.
- **Breadcrumbs:** `src/components/layout/Breadcrumbs.tsx` (sunucu bileşeni + `BreadcrumbList` şeması). Ürün sayfası (`Anasayfa/Kategori/Ürün`) + anasayfada kategori/arama filtresi aktifken.
- **SSS sayfası:** `/sss` — 5 bölüm (ödeme, kargo, iade, ürünler, hesap), `<details>` akordeon + `FAQPage` şeması. Footer + sitemap'e eklendi. Değerler `LEGAL`'den okunuyor.
- **Sepet "kasa altı" önerileri:** `CartDrawer` açılınca `/api/suggestions?exclude=<productId,...>` → `getCartSuggestions()` (aktif + stokta, sepettekiler hariç, kampanyalı/indirimli önce, en ucuz stok varyantı hızlı-ekle hedefi). Toplam bloğunun hemen üstünde yatay kaydırmalı şerit (6 ürün, ince scrollbar + snap), "+ Ekle" butonu (`add_to_cart` olayı da gönderir).
- **Ürün detay "Benzer Ürünler":** `getRelatedProducts(categorySlug, excludeId, 4)` — aynı kategoriden karışık, stokta. Ürün sayfasının en altında `<ProductCard>` ızgarası.
- **İletişim sayfası:** `/iletisim` — adres, çalışma saatleri (`LEGAL.calismaSaatleri`), telefon/WhatsApp/e-posta, Google Maps embed iframe (`output=embed`, API key yok) + "Yol Tarifi Al" butonu. CSP `frame-src`'e `www.google.com` + `maps.google.com` eklendi. Footer İletişim sütununa link. Store şemasına `hasMap` + `openingHours` eklendi. `LEGAL`'e `calismaSaatleri` + `yanitSuresi` alanları (düzenlenebilir yer tutucu).
...
- **2026 denetim raporu düzeltmeleri (1-2/3):**
  - **Kullanılmayan paketler kaldırıldı:** `@tanstack/react-query`, `@tanstack/react-query-devtools`, `clsx` — hiçbir yerde `useQuery` yoktu. `Providers` artık sadece `CartHydrator` (server component).
  - **`ProductCard` → server component:** görsel/rozet/ad sunucuda; etkileşimli alt blok (varyant + canlı fiyat + Sepete Ekle) → `ProductCardActions.tsx` client island. Görsel üstü indirim/stok rozetleri artık **ilk varyanttan** hesaplanıyor (varyant değişince rozet sabit, fiyat canlı — çoğu ürün tek varyant).
  - **Erişilebilirlik:** `src/hooks/useDialog.ts` (Esc + odak tuzağı + body scroll-lock + kapalıyken `inert`) → `CartDrawer` + `MobileDrawer` `role="dialog"` `aria-modal`. Kategori menüsüne Esc. Checkout + `AddressManager` formlarına gerçek `<label>` + `autoComplete` (`.chk-label`/`.adr-label`). `globals.css`: `:focus-visible` halkası + `prefers-reduced-motion`. Layout'a "İçeriğe geç" skip-link + `<main id="main-content">`. Anasayfadaki çift `<main>` → `<section>`.
  - **Güvenlik başlıkları:** `next.config.mjs` → `Strict-Transport-Security` (HSTS, 2 yıl, preload). Admin layout → `metadata.robots = { index:false, follow:false }`.
  - **YAPILMADI (ayrı oturum):** CSP `'unsafe-inline'` → nonce geçişi. Riskli (tüm script'ler: Turnstile/iyzico/GA/Sentry), dev sunucusunda test şart. `npm audit`: 36 açık (çoğu dev bağımlılığı) — ayrıca bakılacak.
- **Admin ürün listesi arama/filtre:** `/admin/urunler` — `ProductListFilters.tsx` (client): ad araması (300ms debounce), kategori / durum (aktif-pasif) / stok (var-azalan-yok) / sıralama (yeni-isim-stok). Filtre durumu URL'de (`q`/`kategori`/`durum`/`stok`/`sirala`); ad+kategori+durum DB'de, stok bucket + `stok-az` sıralama bellekte (varyant toplamı PostgREST'te filtrelenemiyor). Kategori sütunu tabloya eklendi. Migration yok.
- **Ürün galerisi (migration 0030):** `products.image_paths text[]` — ana görsel `image_path` değişmedi (kart/sepet/OG/Schema hâlâ onu kullanır), `image_paths` EK galeri (sıralı, ≤8). `ProductGallery.tsx` ürün detayında dokunmatik kaydırmalı galeri (scroll-snap + ok + nokta + küçük görsel şeridi); tek görselde sade kare. `getProductImageUrls(main, extras)` helper (`media.ts`). Schema.org `image` artık dizi. Admin `ProductForm` "Diğer Görseller (galeri)" bölümü: çoklu yükleme (ortak `uploadFile()` helper'ına refactor edildi), ←/→ sıralama, kaldırma. `productInputSchema.imagePaths` + `productColumns` + `parseProductFormData` (`imagePathsJson`) + `duzenle` sorgusu güncel. `types.ts` products `Row.image_paths: string[]`, `Insert` opsiyonel.
