...
export default async function OrderDetailPage({ params }: OrderDetailPageProps) {
  ...
  const { data: order } = await supabase
    .from('orders')
    .select(
      'id, order_number, status, subtotal_cents, shipping_cents, total_cents, currency, shipping_address, billing_address, contact_email, contact_phone, payment_provider, payment_ref, shipping_carrier, tracking_number, shipped_at, created_at, updated_at, order_items(id, product_name_snapshot, variant_label_snapshot, unit_price_cents, quantity)'
    )
    .eq('id', id)
    .single();
